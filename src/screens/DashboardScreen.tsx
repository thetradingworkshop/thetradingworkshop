  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-12 pb-12">
      ...
      <Card className="p-0 overflow-hidden border-border/50 shadow-xl bg-card">
        {/* HEADER BAR */}
        <div className="p-2.5 border-b border-border/50 bg-card flex flex-col md:flex-row md:items-center justify-between gap-2.5">
          <div className="flex items-center space-x-2.5">
            <div className="flex items-center space-x-1">
              <Button variant="ghost" size="sm" onClick={prevMonth} className="h-6 w-6 p-0 hover:bg-muted"><ChevronLeft className="w-3 h-3" /></Button>
              <h3 className="text-xs font-bold text-foreground min-w-[88px] text-center">{monthYearLabel}</h3>
              <Button variant="ghost" size="sm" onClick={nextMonth} className="h-6 w-6 p-0 hover:bg-muted"><ChevronRight className="w-3 h-3" /></Button>
            </div>
            <Button
              variant="outline"
              size="sm"
              onClick={resetMonth}
              className="px-2.5 h-7 text-[10px] font-medium rounded-xl border-border hover:bg-muted"
            >
              This month
            </Button>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold text-foreground">Monthly stats:</span>
              <div className="flex items-center gap-1.5">
                <Badge variant="neutral" className={cn(
                  "bg-emerald-50 text-emerald-600 border-none text-[10px] font-bold h-5 px-2",
                  rangeStats.totalPnL < 0 && "bg-rose-50 text-rose-600"
                )}>
                  {rangeStats.totalPnL >= 0 ? '+' : '-'}${Math.abs(rangeStats.totalPnL).toLocaleString()}
                </Badge>
                <Badge variant="neutral" className="bg-muted text-muted-foreground border-none text-[10px] font-bold h-5 px-2">
        ...
        <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-sm">
          <div className={cn(
            ...
            calendarSettings.showWeeklyPanel
              ? (calendarSettings.showWeekends ? "grid-cols-[repeat(7,1fr)_110px]" : "grid-cols-[repeat(5,1fr)_110px]")
            ...
            {(calendarSettings.showWeekends ? ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] : ['Mon', 'Tue', 'Wed', 'Thu', 'Fri']).map(day => (
              <div key={day} className="bg-muted/50 border border-border rounded-lg py-1 text-center">
                <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-wider">{day}</span>
              </div>
            ))}
            {calendarSettings.showWeeklyPanel && (
              <div className="flex items-center justify-between px-1.5">
                <h4 className="text-[8px] font-black uppercase tracking-[0.15em] text-muted-foreground">Weekly</h4>
                ...
                {row.map((d, i) => {
                  ...
                  return (
                    <div 
                      ...
                      className={cn(
                        "group relative h-[68px] rounded-xl border transition-all duration-300 cursor-pointer overflow-hidden",
                      ...
                      (() => {
                        ...
                        return (
                          <div 
                            ...
                            className={cn(
                              "group h-full p-1.5 bg-muted/50 border border-border rounded-xl transition-all duration-300 cursor-pointer hover:border-indigo-400 hover:shadow-lg relative overflow-hidden flex flex-col justify-between",
